...
import { UserPlus, ArrowLeft, User, Mail, Phone, Lock, AlertCircle } from "lucide-react";
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await signup(name.trim(), normalizedEmail, phone.trim(), password);
      router.push(`/login?registered=1&uid=${encodeURIComponent(normalizedEmail)}`);
    } catch (err) {
    ...
    >
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" disabled={isPending || !name || !email || !phone || !password || !confirmPassword} className={primaryButton}>
