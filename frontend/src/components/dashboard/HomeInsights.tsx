...
export function buildHomeInsights(summary: DashboardSummary, cur: (v: number) => string): Insight[] {
  ...
  if (k.transactionCount === 0) {
    return [{ id: "empty", icon: "✨", text: "Log your first transaction to unlock personalized financial analysis here.", tone: "neutral" }];
  }

  // Income vs. expenses / savings rate
  if (Number.isFinite(k.savingsRatePct)) {
    if (k.savingsRatePct >= 20) {
      insights.push({ id: "savings-good", icon: "💪", text: `Savings rate is ${k.savingsRatePct.toFixed(0)}% of income this period — comfortably above the 20% benchmark.`, tone: "positive" });
    } else if (k.savingsRatePct >= 0) {
      insights.push({ id: "savings-ok", icon: "📈", text: `Savings rate is ${k.savingsRatePct.toFixed(0)}% of income this period, below the commonly recommended 20% target.`, tone: "neutral" });
    } else {
      insights.push({ id: "savings-negative", icon: "⚠️", text: `Expenses currently exceed income (${k.savingsRatePct.toFixed(0)}% savings rate) — a spending review is worth prioritizing.`, tone: "critical" });
    }
  }

  // Spending trend / concentration
  if (k.highestSpendingCategory) {
    insights.push({ id: "top-category", icon: "🏷️", text: `${k.highestSpendingCategory} is your largest spending category — the first place to look for savings opportunities.`, tone: "neutral" });
  }

  // Unusually high / largest expense
  if (k.largestExpense > 0) {
    insights.push({ id: "largest-expense", icon: "🧾", text: `The largest single expense on record is ${cur(k.largestExpense)}.`, tone: "neutral" });
  }

  // Budget deviation
  if (Number.isFinite(k.budgetUtilizationPct) && k.budgetUtilizationPct > 0) {
    if (k.budgetUtilizationPct >= 90) {
      insights.push({ id: "budget-high", icon: "🚨", text: `${k.budgetUtilizationPct.toFixed(0)}% of budget has been used this month — spending is tracking ahead of plan.`, tone: "warning" });
    } else if (k.budgetUtilizationPct <= 50) {
      insights.push({ id: "budget-comfortable", icon: "✅", text: `Budget utilization is ${k.budgetUtilizationPct.toFixed(0)}% for the month — well within plan.`, tone: "positive" });
    }
  }
  ...
  if (Number.isFinite(k.emergencyFundProgressPct) && k.emergencyFundProgressPct > 0 && k.emergencyFundProgressPct < 100) {
    insights.push({ id: "emergency-fund", icon: "🛟", text: `Emergency fund progress stands at ${k.emergencyFundProgressPct.toFixed(0)}% of target.`, tone: "neutral" });
  }
  ...
  if (Number.isFinite(k.investmentGrowth) && k.investmentGrowth !== 0) {
    ...
    insights.push({ id: "investment-growth", icon: up ? "📊" : "📉", text: `Investment portfolio is ${up ? "up" : "down"} ${Math.abs(k.investmentGrowth).toFixed(1)}% overall.`, tone: up ? "positive" : "warning" });
  }
