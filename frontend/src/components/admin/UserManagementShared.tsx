...
export interface UsageCounts {
  // Financial-model counts aren't included — that data lives solely in the user's own Google
  // Drive, which the platform has no visibility into by design.
...
const USAGE_LABELS: Record<keyof UsageCounts, string> = {
  notifications: "Notifications", activityLogs: "Activity Log Entries",
};
...
export function UsageModal({ user, onClose }: { user: ManagedUser; onClose: () => void }) {
  ...
  return (
    <ModalShell onClose={onClose}>
      <p className="text-sm font-semibold text-navy dark:text-white">Account activity for {user.name}</p>
      <p className="mt-1 text-xs text-navy/40 dark:text-white/40">Financial data (transactions, budgets, etc.) lives in this user&apos;s own Google Drive and isn&apos;t visible to the platform.</p>
      {isLoading ? (
